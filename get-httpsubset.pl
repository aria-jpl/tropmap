...
#  print "   * note * gfs-avn became gfs, and meso-eta became nam on Feb 15, 2005\n\n";
  ...
  while( $dataset ne "narr-a"      && $dataset ne "narr-b" &&
         ...
         $dataset ne "namanl" && $dataset ne "gfsanl" && $dataset ne "rucanl" &&
         $dataset ne "nnr2"  
       ) 
     {
     print " DATASET : ";
...
if( $dataset ne "narr-a" && $dataset ne "narr-b" && $dataset ne "narrmon-a" && 
		...
		$dataset ne "namanl" && $dataset ne "gfsanl" && $dataset ne "ruc20" && $dataset ne "rucanl")
	{
...
if( $dataset =~ m/^rucanl$/ )
   { $DATADIR = "rucanl";    $modelName = "ruc2anl";
     $mode = "hourly"; $GRID=252; $HR_INC=1; 
   } 
if( $dataset =~ m/^nnr2$/ )
   {
...
#print "Grepping for $grepPattern\n";
